...
Safe Apply decisions wait at most two seconds for a concurrent watchdog observation within the existing IPC budget, then recheck the original receipt under the transaction lock. Concurrent copies of a decision receive one durable result. A stale transaction sequence or settled transaction is an explicit `not-started` decision rejection; the user reviews fresh evidence before choosing again. Lock-budget exhaustion and transport failures remain unknown because another copy may still be running. The frontend never automatically resubmits a decision.
