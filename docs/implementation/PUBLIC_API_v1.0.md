...
动态 JSON 另有解析预算：深度 16、数组/对象最多 4096 项、16384 tokens、单字符串 64 KiB、数值词法最多 64 字节且指数范围 -308…308。先检查预算再调用精确数值 schema 校验，避免极端指数展开；管理 ID、uint64 sequence 继续用字符串，正常数值不做浮点舍入。
